...
def updateqty(req,qv,productid):
    ...
    if qv==1:
        ...
        allcarts.update(qty=total) #setting 0 coz at a time only one cart will be accessed
    else:
        if allcarts[0].qty>1:
            total=allcarts[0].qty-1
...
def addcart(req,productid):
    ...
    cartitems,created=Cart.objects.get_or_create(productid=allproducts,userid=user)  
    if not created:
        cartitems.qty+=1  
    else:
        cartitems.qty=1  
...
def payment(req):
    if req.user.is_authenticated:
        ...
        #     totalamount+=x.qty*x.productid.price   
        #     oid=x.orderid 
        client = client = razorpay.Client(auth=("rzp_test_wH0ggQnd7iT3nB", "eZseshY3oSsz2fcHZkTiSlCm"))
